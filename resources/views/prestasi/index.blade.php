...
<body>
  ...
  <nav class="fixed top-0 left-0 right-0 bg-slate-900/80 backdrop-blur-md border-b border-slate-700 p-4 flex justify-between items-center px-6 z-50">
    ...
    <button id="logoutBtn" class="logout-btn text-white font-semibold py-2 px-5 rounded-lg">
      Keluar
    </button>
  </nav>
  ...
  <div class="card fade-in mt-10">
    ...
    <form id="prestasiForm" class="mb-8 grid grid-cols-1 md:grid-cols-5 gap-4">
      ...
      <input type="text" id="kelas" placeholder="Kelas (contoh: XII IPA 1)" required
        class="p-3 rounded-lg bg-slate-800 text-gray-200 border border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none">
      <input type="text" id="jurusan" placeholder="Jurusan (contoh: IPA / IPS / RPL)" required
        class="p-3 rounded-lg bg-slate-800 text-gray-200 border border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none">
      <input type="text" id="prestasi" placeholder="Prestasi (contoh: Juara 1 Olimpiade)" required
        class="p-3 rounded-lg bg-slate-800 text-gray-200 border border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none">
      <input type="text" id="tahun" placeholder="Tahun" required
        class="p-3 rounded-lg bg-slate-800 text-gray-200 border border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none">
      
      <button type="submit"
        class="col-span-1 md:col-span-5 glow-btn text-white py-3 rounded-lg font-semibold">
        Tambahkan Prestasi
      </button>
    </form>
    ...
    <div class="overflow-x-auto">
      <table class="w-full border-collapse">
        ...
        <tbody id="tabelPrestasi" class="divide-y divide-slate-700 bg-slate-800 text-gray-300">
  ...
  <script>
    ...
    const tabel = document.getElementById('tabelPrestasi');
    ...
    form.addEventListener('submit', (e) => {
      ...
      if (nama && kelas && jurusan && prestasi && tahun) {
        ...
        row.innerHTML = `
          ...
          <td class="py-3 px-4">${prestasi}</td>
          <td class="py-3 px-4">${tahun}</td>
          <td class="py-3 px-4 text-center">
            <button onclick="hapusBaris(this)" class="bg-red-500 hover:bg-red-600 text-white py-1 px-3 rounded-lg transition">
              Hapus
            </button>
          </td>
        `;
    ...
    btnKeluar.addEventListener('click', () => {
      popupLogout.classList.add('hidden');
      window.location.href = "login.html"; // ubah ke halaman login
    });
  </script>
</body>
</html>